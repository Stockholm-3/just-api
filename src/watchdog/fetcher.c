...
int fetch_all_price_groups_sync(const char* output_path, const char* port,
    ...
    for (size_t i = 0; i < COUNT; i++) {
        ...
        if (json_is_array(root)) {
            ...
            json_array_foreach(root, j, v) {

                if (json_array_append(merged, v) != 0) {
                    fprintf(stderr, "json_array_append failed\n");
                }
            }
        } else if (json_is_object(root)) {

            if (json_array_append(merged, root) != 0) {
                fprintf(stderr, "json_array_append failed\n");
            }
        } else {
    ...
    char* dump = json_dumps(merged, JSON_INDENT(2));

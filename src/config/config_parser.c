...
void config_set_defaults(ServerConfig* config) {
    ...
    config->cache.weather_ttl_seconds = 900;
    config->cache.geo_ttl_seconds     = 604800;
    config->cache.enabled             = true;

    config->geocoding.max_results = 10;
    strncpy(config->geocoding.language, "en",
    ...
    strncpy(config->energy_plan.base_dir, "energy_plan",
            sizeof(config->energy_plan.base_dir) - 1);
    config->energy_plan.max_cities       = 200;
    config->energy_plan.city_ttl_seconds = 2L * 24 * 3600;

    strncpy(config->scheduler.service_host, "127.0.0.1",
            sizeof(config->scheduler.service_host) - 1);
    strncpy(config->scheduler.weather_url_path, "/v1/forecast/minutely",
            sizeof(config->scheduler.weather_url_path) - 1);
    strncpy(config->scheduler.elpris_url_path, "/v1/elpris",
            sizeof(config->scheduler.elpris_url_path) - 1);
    config->scheduler.price_zones_count = 4;
    strncpy(config->scheduler.price_zones[0], "SE1", 7);
    strncpy(config->scheduler.price_zones[1], "SE2", 7);
    strncpy(config->scheduler.price_zones[2], "SE3", 7);
    strncpy(config->scheduler.price_zones[3], "SE4", 7);
    config->scheduler.timeout_ms          = 10000;
    config->scheduler.weather_interval_ms = 60ULL * 60 * 1000; // 1 hour
    config->scheduler.weather_offset_ms   = 60ULL * 2 * 1000;  // HH:02:00
    config->scheduler.elpris_hour_utc     = 13;
    config->scheduler.elpris_minute_utc   = 5;
    ...
    strncpy(config->compute.log_dir, "logs/energy_parser",
            sizeof(config->compute.log_dir) - 1);

    strncpy(config->paths.server_binary, "./just-server",
            sizeof(config->paths.server_binary) - 1);
    strncpy(config->paths.compute_binary, "./compute",
            sizeof(config->paths.compute_binary) - 1);
    strncpy(config->paths.log_dir, "./logs", sizeof(config->paths.log_dir) - 1);
}
...
int config_parser_load(const char* filepath, ServerConfig* config) {
    ...
    if (s) {
        INT_SET(config->server_port, json_object_get(s, "port"));
        INT_SET(config->max_connections, json_object_get(s, "max_connections"));
        BOOL_SET(config->daemon_mode, json_object_get(s, "daemon_mode"));
    }

    json_t* c = json_object_get(root, "cache");
    if (c) {
        STR_SET(config->cache.cache_dir, json_object_get(c, "directory"));
        INT_SET(config->cache.weather_ttl_seconds,
                json_object_get(c, "weather_ttl_seconds"));
        INT_SET(config->cache.geo_ttl_seconds,
                json_object_get(c, "geo_ttl_seconds"));
        BOOL_SET(config->cache.enabled, json_object_get(c, "enabled"));
    }

    json_t* g = json_object_get(root, "geocoding");
    if (g) {
        INT_SET(config->geocoding.max_results,
                json_object_get(g, "max_results"));
        STR_SET(config->geocoding.language, json_object_get(g, "language"));
    }

    /* --- watchdog --- */
    json_t* w = json_object_get(root, "watchdog");
    if (w) {
        STR_SET(config->watchdog.pid_file, json_object_get(w, "pid_file"));
        INT_SET(config->watchdog.max_restarts,
                json_object_get(w, "max_restarts"));
        INT_SET(config->watchdog.restart_window_sec,
                json_object_get(w, "restart_window_sec"));
        INT_SET(config->watchdog.initial_backoff_ms,
                json_object_get(w, "initial_backoff_ms"));
        INT_SET(config->watchdog.max_backoff_ms,
                json_object_get(w, "max_backoff_ms"));
        INT_SET(config->watchdog.server_ready_wait_ms,
    ...
    if (tp) {
        INT_SET(config->thread_pool.num_workers,
                json_object_get(tp, "num_workers"));
        INT_SET(config->thread_pool.max_pending,
                json_object_get(tp, "max_pending"));
    }

    json_t* ep = json_object_get(root, "energy_plan");
    if (ep) {
        STR_SET(config->energy_plan.base_dir, json_object_get(ep, "base_dir"));
        INT_SET(config->energy_plan.max_cities,
                json_object_get(ep, "max_cities"));
        LONG_SET(config->energy_plan.city_ttl_seconds,
                 json_object_get(ep, "city_ttl_seconds"));
    }

    json_t* sc = json_object_get(root, "scheduler");
    if (sc) {
        STR_SET(config->scheduler.service_host,
                json_object_get(sc, "service_host"));
        STR_SET(config->scheduler.weather_url_path,
                json_object_get(sc, "weather_url_path"));
        STR_SET(config->scheduler.elpris_url_path,
                json_object_get(sc, "elpris_url_path"));
        LONG_SET(config->scheduler.timeout_ms,
                 json_object_get(sc, "timeout_ms"));
        LONG_SET(config->scheduler.weather_interval_ms,
                 json_object_get(sc, "weather_interval_ms"));
        LONG_SET(config->scheduler.weather_offset_ms,
                 json_object_get(sc, "weather_offset_ms"));
        INT_SET(config->scheduler.elpris_hour_utc,
                json_object_get(sc, "elpris_hour_utc"));
        INT_SET(config->scheduler.elpris_minute_utc,
        ...
        if (zones && json_is_array(zones)) {
            ...
            config->scheduler.price_zones_count = n;
            for (int i = 0; i < n; i++) {
                json_t* z = json_array_get(zones, i);
                if (z && json_is_string(z)) {
                    strncpy(config->scheduler.price_zones[i],
    ...
    if (co) {
        STR_SET(config->compute.cities_csv, json_object_get(co, "cities_csv"));
        STR_SET(config->compute.compute_input_dir,
                json_object_get(co, "compute_input_dir"));
        STR_SET(config->compute.elpris_json,
                json_object_get(co, "elpris_json"));
        STR_SET(config->compute.output_dir, json_object_get(co, "output_dir"));
        STR_SET(config->compute.lock_file, json_object_get(co, "lock_file"));
        STR_SET(config->compute.log_dir, json_object_get(co, "log_dir"));
    }

    json_t* pa = json_object_get(root, "paths");
    if (pa) {
        STR_SET(config->paths.server_binary,
                json_object_get(pa, "server_binary"));
        STR_SET(config->paths.compute_binary,
                json_object_get(pa, "compute_binary"));
        STR_SET(config->paths.log_dir, json_object_get(pa, "log_dir"));
    }
...
int config_parser_validate(const ServerConfig* config) {
    ...
    if (config->energy_plan.max_cities <= 0) {
        fprintf(stderr, "[CONFIG] energy_plan.max_cities must be > 0\n");
        return -1;
    }
    if (config->scheduler.price_zones_count <= 0) {
...
void config_parser_print(const ServerConfig* config) {
    ...
    printf("  initial_backoff_ms:    %d\n",
           config->watchdog.initial_backoff_ms);
    ...
    printf("Energy plan:\n");
    printf("  base_dir:         %s\n", config->energy_plan.base_dir);
    printf("  max_cities:       %d\n", config->energy_plan.max_cities);
    printf("  city_ttl_seconds: %ld\n", config->energy_plan.city_ttl_seconds);

    printf("Scheduler:\n");
    printf("  service_host:        %s\n", config->scheduler.service_host);
    printf("  weather_url_path:    %s\n", config->scheduler.weather_url_path);
    printf("  elpris_url_path:     %s\n", config->scheduler.elpris_url_path);
    printf("  price_zones:         ");
    for (int i = 0; i < config->scheduler.price_zones_count; i++) {
        printf("%s%s", config->scheduler.price_zones[i],
               i + 1 < config->scheduler.price_zones_count ? ", " : "\n");
    }
    printf("  timeout_ms:          %ld\n", config->scheduler.timeout_ms);
    printf("  weather_interval_ms: %ld\n",
           config->scheduler.weather_interval_ms);
    printf("  weather_offset_ms:   %ld\n", config->scheduler.weather_offset_ms);
    printf("  elpris_hour_utc:     %d\n", config->scheduler.elpris_hour_utc);
    printf("  elpris_minute_utc:   %d\n", config->scheduler.elpris_minute_utc);
    ...
    printf("  log_dir:           %s\n", config->compute.log_dir);

    printf("Paths:\n");
    printf("  server_binary:  %s\n", config->paths.server_binary);
    printf("  compute_binary: %s\n", config->paths.compute_binary);

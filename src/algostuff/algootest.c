...
int Sell_Or_Not(float current_price, float celcius);
...
int run_weather_analysis(const char* price_file, const char* weather_file) {
    ...
    int num_temps;
    float* weatherData = extract_temperatures(tmp_json, &num_temps);
    free(tmp_json);
    
    if (weatherData == NULL || num_temps <= 0) {
    ...
    float current_temp = get_value_by_index(weatherData, num_temps, index);
...
int main() {
    return run_weather_analysis("elPrisdata.txt", "vaderData.txt");
}
...
int calculate_current_index(void) {
    return get_current_15min_index();
}
...
float estimate_solar_production(int hour, float current_temp) {
    ...
    if (current_temp > 25.0f) {
        ...
        if (temp_factor < 0.7f) temp_factor = 0.7f;  // Minst 70% kvar
    }

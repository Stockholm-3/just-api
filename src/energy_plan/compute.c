...
static void build_weather_path(const CityData* c, const char* input_dir,
    ...
    snprintf(out, out_len, "%s/%s-%f-%f.json", input_dir, name_lower,
             c->latitude, c->longitude);
}

static int write_output(const CityData* c, const bool decisions[SLOTS_PER_DAY],
                        const char* output_dir) {
    char zone_str[16];
    snprintf(zone_str, sizeof(zone_str), "SE%d", c->price_zone + 1);

    char path[MAX_FILENAME];
    snprintf(path, sizeof(path), "%s/%s-%s.json", output_dir, c->name,
             zone_str);
    ...
    snprintf(tmp_path, sizeof(tmp_path), "%s/.%s-%s.tmp", output_dir, c->name,
             zone_str);
...
void compute_config_set_defaults(ComputeConfig* cfg) {
    strncpy(cfg->cities_csv, "energy_plan/cities.csv",
            sizeof(cfg->cities_csv) - 1);
    strncpy(cfg->compute_input_dir, "cache/compute_input",
            sizeof(cfg->compute_input_dir) - 1);
    strncpy(cfg->elpris_json, "cache/compute_input/elpris_merged.json",
            sizeof(cfg->elpris_json) - 1);
    strncpy(cfg->output_dir, "energy_plan/compute_output",
            sizeof(cfg->output_dir) - 1);
    strncpy(cfg->lock_file, "energy_plan/compute_output/.lock",
            sizeof(cfg->lock_file) - 1);
}

int compute_run(const ComputeConfig* cfg) {
    LOG_INFO(LOG_MODULE, "=== Energy compute starting ===");
    ...
    int lock_fd = acquire_exclusive_lock(cfg->lock_file);
    if (lock_fd < 0) {
    ...
    int      city_count = load_cities(cfg->cities_csv, cities, MAX_CITIES);
    if (city_count <= 0) {
    ...
    if (load_elpris(cfg->elpris_json, &elpris) <= 0) {
    ...
    if (clear_output_directory(cfg->output_dir) != 0) {
        LOG_WARN(LOG_MODULE,
...

    {
        ...
        for (int ci = 0; ci < city_count; ci++) {
            ...
            build_weather_path(c, cfg->compute_input_dir, weather_path,
                               sizeof(weather_path));

            int wcount = load_weather(weather_path, c->weather, SLOTS_PER_DAY);
            if (wcount < 0) {
                LOG_ERROR(LOG_MODULE,
                          "Skipping '%s' -- weather file not found: %s",
                          c->name, weather_path);
                fail++;
                continue;
            }

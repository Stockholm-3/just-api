/**
 * @file api_server.h
 * @brief api HTTP server main module.
 *
 * This module provides the main apiServer structure and lifecycle
 * management functions. The apiServer wraps an HTTP server and
 * manages multiple client connections through apiServerInstance objects.
 *
 * @par Architecture:
 * - apiServer contains an HTTPServer for handling TCP connections
 * - Each client connection creates a apiServerInstance
 * - A scheduler task performs periodic work on all active instances
 * - Instances are stored in a linked list for iteration
 *
 * @par Usage:
 * @code{.c}
 * apiServer* server = NULL;
 * if (api_server_initiate_ptr(&server) == 0) {
 *     // Server is running, handle events...
 *     api_server_dispose_ptr(&server);
 * }
 * @endcode
 *
 * @see api_server_instance.h for individual connection handling
 * @see http_server.h for the underlying HTTP server implementation
 */

#ifndef API_SERVER_H
#define API_SERVER_H

#include "http_server.h"
#include "linked_list.h"
#include "smw.h"
#include "thread_pool.h"

/**
 * @brief Main api server structure.
 *
 * Contains the HTTP server, connection instances, and scheduler task
 * for managing the api API server.
 */
typedef struct {
    /** @brief Embedded HTTP server for handling connections. */
    HTTPServer httpServer;

    /** @brief Linked list of active apiServerInstance pointers. */
    LinkedList* instances;

    /** @brief Scheduler task for periodic instance work. */
    SmwTask* task;

    /** @brief Thread pool for offloading blocking request work. */
    ThreadPool* request_pool;

} ApiServer;

/**
 * @brief Initialize a stack-allocated apiServer.
 *
 * Performs complete initialization of the api server:
 * - Initializes the embedded HTTP server with connection callback
 * - Creates an empty linked list for client instances
 * - Registers a scheduler task for periodic work
 *
 * @param[in,out] server Pointer to the apiServer structure to initialize.
 *                       Must be valid, non-NULL memory.
 *
 * @return 0 on success, non-zero on failure.
 *
 * @note The server must be disposed with api_server_dispose() when done.
 *
 * @par Example:
 * @code{.c}
 * apiServer server;
 * if (api_server_initiate(&server) == 0) {
 *     // Use server...
 *     api_server_dispose(&server);
 * }
 * @endcode
 */
int api_server_initiate(ApiServer* server, ThreadPool* request_pool);

/**
 * @brief Allocate and initialize a apiServer dynamically.
 *
 * Allocates memory for a apiServer and initializes it using
 * api_server_initiate(). On initialization failure, the allocated
 * memory is automatically freed.
 *
 * @param[out] server_ptr Pointer to receive the allocated server.
 *                        Set to the new server on success, unchanged on
 * failure.
 *
 * @return
 * - 0 on success
 * - -1 if server_ptr is NULL
 * - -2 if memory allocation fails
 * - Other non-zero values from api_server_initiate() on init failure
 *
 * @note The server must be disposed with api_server_dispose_ptr() when
 * done.
 *
 * @par Example:
 * @code{.c}
 * apiServer* server = NULL;
 * int result = api_server_initiate_ptr(&server);
 * if (result == 0) {
 *     // Use server...
 *     api_server_dispose_ptr(&server);
 * }
 * @endcode
 */
int api_server_initiate_ptr(ApiServer** server_ptr, ThreadPool* request_pool);

/**
 * @brief Shut down and clean up a stack-allocated apiServer.
 *
 * Performs complete cleanup of the api server:
 * - Disposes all active client instances
 * - Frees the instances linked list
 * - Stops and disposes the HTTP server
 * - Destroys the scheduler task
 *
 * @param[in] server Pointer to the apiServer to dispose.
 *                   Must have been initialized with api_server_initiate().
 *
 * @note After calling this function, the server structure should not be used
 *       unless re-initialized.
 */
void api_server_dispose(ApiServer* server);

/**
 * @brief Dispose and free a dynamically allocated apiServer.
 *
 * Calls api_server_dispose() to clean up the server, then frees
 * the allocated memory and sets the pointer to NULL.
 *
 * @param[in,out] server_ptr Pointer to the apiServer pointer.
 *                           The pointed-to pointer is set to NULL after
 * disposal. Safe to call with NULL or pointer to NULL.
 *
 * @par Example:
 * @code{.c}
 * apiServer* server = NULL;
 * api_server_initiate_ptr(&server);
 * // Use server...
 * api_server_dispose_ptr(&server);
 * // server is now NULL
 * @endcode
 */
void api_server_dispose_ptr(ApiServer** server_ptr);

#endif // API_SERVER_H
